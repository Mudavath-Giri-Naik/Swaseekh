  ...
  const ParallaxCardItem = ({ item, id }: { item: typeof cardItems[0]; id: number }) => {
    ...
    return (
      <ParallaxCardEffect
        ...
        className={cn(
          "relative flex flex-col justify-center rounded-xl md:rounded-2xl px-4 py-6 md:px-12 md:py-12 w-[88vw] max-w-[700px] min-h-[200px] md:min-h-[300px]",
          item.className
        )}>
        <div className="flex flex-col items-center justify-center text-center space-y-3 md:space-y-6 h-full">
          <h3 className="text-xl sm:text-3xl md:text-4xl font-bold tracking-tight">
            {item.title}
          </h3>
          <p className="text-xs sm:text-base md:text-lg opacity-80 max-w-[600px] leading-relaxed">
            <strong className="font-semibold block mb-0.5 md:mb-1">{item.headline}</strong>
            {item.description}
          </p>
          <div className="pt-2">
