export const meta = {
  name: 'v3-sheets-s1',
  description: 'Phase2 shard 1/1: author formula sheets for 8 concepts',
...
const TODO = ["con_055","con_061","con_064","con_065","con_066","con_067","con_068","con_069","con_070","con_071","con_072","con_073","con_074","con_075","con_076","con_077","con_078","con_079","con_080","con_081","con_082"]
phase('Author')
// Low concurrency (3 at a time) to stay under the server rate limit.
const results = []
const CHUNK = 3
for (let i = 0; i < TODO.length; i += CHUNK) {
  const chunk = TODO.slice(i, i + CHUNK)
  const r = await parallel(chunk.map((cid) => () =>
    agent(prompt(cid), { label: `sheet:${cid}`, phase: 'Author', schema: SCHEMA, model: 'sonnet' })
  ))
  results.push(...r)
  log(`chunk ${i / CHUNK + 1}: ${r.filter(Boolean).length}/${chunk.length} ok (${results.filter(Boolean).length}/${TODO.length} total)`)
}
